...
<h3><b><font face='Algerian'>Click to Visit the Forums You have interacted with</font></b></h3>
<hr>
<br>
<br>
<br>
...
$nocv_id=$_GET['nocv_id'];
 ...
 if($bitch==$nocv_id){
$select=mysqli_query($db,"SELECT * FROM replied WHERE nocv_id='$bitch' ORDER BY 1 DESC LIMIT 0,100 ") or die(mysqli_error($db));
while($row=mysqli_fetch_array($select)){
    ...
    $topic=$row['topic'];
 $select_general=mysqli_query($db,"SELECT * FROM announcement WHERE nocv_id='$start_chat'") or die(mysqli_error($db));   
while($row_general=mysqli_fetch_array($select_general)){
    
    $nocv_idd=$row_general['nocv_id'];
    $comment=$row_general['comment'];
    $status=$row_general['status']; 
    $chat=substr($nocv_idd,-3,2);
    $datee=$row_general['date'];

    if($bitch!==$nocv_idd){
       echo"  
<h4  style='word-break: break-all'><b>$comment</b></h4>
<p>$datee</p>";
echo "<a href='general.php?nocv_id=$nocv_idd&start_chat=$nocv_idd&chat=$chat&comment=$comment&status=$status'class='btn btn-primary glyphicon glyphicon-search'>GO</a>
<p align='right'><a href='delete_replied.php?id=$id' class='btn btn-danger glyphicon glyphicon-trash'>Delete</a></p>
<hr>";  
