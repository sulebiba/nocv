...
<?php
    ...
    $upload1=$_GET['upload1'];
    $select=mysqli_query($db,"SELECT * FROM uploads WHERE upload1='$upload1'")or die(mysqli_error($db)); 
    if(mysqli_num_rows($select)>0){
    while($result=mysqli_fetch_array($select)){
    $nocv_id=$result['nocv_id'];
    $select2=mysqli_query($db,"SELECT * FROM users WHERE nocv_id='$nocv_id'")or die(mysqli_error($db));
...
?>
   <img src="users/<?php echo $Profile; ?>"width="100%"height="410px"> 
    ...
    <?php
    if(isset($_SESSION['nocv_id'])){
         echo"<th class='col-sm-3 text-center'><b id='greeen'><br><br>FORUM:</b></th><th class='col-sm-3'><br><br>$name</th>";
        }
        else{
            echo"<th class='col-sm-3 text-center'><b id='greeen'><br><br>FORUM:</b></th><th class='col-sm-3'><br><br>$name</th>";
        }
    ...
    <tr>
        ...
        if(isset($_SESSION['nocv_id'])){
        $get = $_SESSION['nocv_id'];
        $select=mysqli_query($db,"SELECT * FROM contactuser WHERE nocv_id='$nocv_id'")or die(mysqli_error($db));
        $number=mysqli_num_rows($select);
        if($get==$nocv_id){
         echo"<th class='col-sm-3 text-center'><b id='greeen'><br><br></b></th><th class='col-sm-3'><br><br><a href='my_messages.php?nocv_id=$get'class='btn btn-primary glyphicon glyphicon-envelope'>INBOX($number)</a></th>";
            }
            else{
            
            echo"<th class='col-sm-3 text-center'><b id='greeen'><br><br></b></th><th class='col-sm-3'><br><br><a href='contact_user.php?nocv_id=$nocv_id'class='btn btn-primary glyphicon glyphicon-envelope'>SendMessage</a></th>";
           }
            }
        else{
...
<?php 
   ...
   <source src='userspic/$upload1' type='video/mp4'>
         ...
         echo"<img src='userspic/$upload1'class='img-res' height='500px'>";
...
$get = $_SESSION['nocv_id'];
if($get==$nocv_id){
echo "<div id='rightpic'><h2 align='center'><form method='post'><input type='submit'name='delete'class='btn btn-danger'value='delete'></form></h2></div>";
}
else{
    echo"<br><br>";
}
...
<div id='scroll-nocv3'>
<textarea name='message' class='form-control' placeholder='write a review'id='review'required></textarea>
<input type='hidden' name='nocv' class='btn btn-primary' value=$nocv_id>
...
$select=mysqli_query($db,"SELECT * FROM post WHERE upload1='$upload1' ORDER BY 1 ASC") or die (mysqli_error($db));
while($row=mysqli_fetch_array($select)){
    $comment=$row['comment'];
    $date=$row['date'];
    $nocv_id=$row['nocv_id'];
$select_user=mysqli_query($db,"SELECT * FROM users WHERE nocv_id='$nocv_id'") or die (mysqli_error($db));
while($row_user=mysqli_fetch_array($select_user)){
    $name=$row_user['Fullname'];
    $skill=$row_user['skill'];
    $nocv_id=$row_user['nocv_id'];
echo"
<div id='comment col-lg-6 col-md-3 col-sm-12 col-xs-12'class='comment col-lg-12 col-md-3 col-sm-12 col-xs-12'>
<p><a href='profile.php?nocv_id=$nocv_id'>$name</a></p>
<br>
<p>$comment</p>
<br>
<p>$date</p>
...
$nocv_id = mysqli_real_escape_string($db,$_POST['nocv']);
$select_insert2=mysqli_query($db,"SELECT * FROM post WHERE upload1='$upload1'");
...
else if(mysqli_num_rows($select_insert2)>200){
    echo"<script>alert('Comment Limit reached For this post')</script>";
}
else{
$insert= mysqli_query($db,"INSERT INTO post (comment,date,nocv_id,upload1) VALUES ('$message','$date','$nocv_id','$upload1')")or die (mysqli_error($db));
header("Location:single_post.php?upload1=$upload1#nocv3");
//header("Refresh:1;single_post.php?upload1=$upload1");
ob_end_flush();
...
if(isset($_GET['upload1'])){
  $upload1=$_GET['upload1'];
  $selectfile=mysqli_query($db,"SELECT * FROM uploads WHERE upload1='$upload1'");
  while($rowfile=mysqli_fetch_array($selectfile)){
     $uppy=$rowfile['upload1']; 
  }
}
if(isset($_POST['delete'])){
   $delete=mysqli_query($db,"DELETE FROM post WHERE upload1='$upload1'") or die(mysqli_error($db));
   $delete=mysqli_query($db,"DELETE FROM uploads WHERE upload1='$upload1'") or die(mysqli_error($db));
   $path="userspic/$uppy";
   unlink($path);
   $nocv_id=$_SESSION['nocv_id'];
   header("Location:profile.php?nocv_id=$nocv_id");
   ob_end_flush();
