...
<h3><b><font face='Algerian'>Click to reply/Delete messages</font></b></h3>
<hr>
<br>
<br>
<br>
...
$nocv_id=$_GET['nocv_id'];
 ...
 if($bitch==$nocv_id){
$select=mysqli_query($db,"SELECT * FROM contactuser WHERE nocv_id='$nocv_id' ORDER BY 1 DESC LIMIT 0,100 ") or die(mysqli_error($db));
while($row=mysqli_fetch_array($select)){
    $phone=$row['phone'];
    $message=$row['message'];
    $date=$row['date'];
    $id=$row['id'];
    $pos=(strpos($phone,'nocv'));
...
<p>$date</p>";
echo "<a href='search_forum.php?forum_id=$phone'class='btn btn-primary glyphicon glyphicon-search'>GO</a>
<p align='right'><a href='delete.php?id=$id' class='btn btn-danger glyphicon glyphicon-trash'>Delete</a></p>
<hr>";  
...
echo "<a href='https://api.whatsapp.com/send?phone=234$new_phone&text=chat%20from%20nocv'class='btn btn-primary glyphicon glyphicon-user'>reply</a>
<p align='right'><a href='delete.php?id=$id' class='btn btn-danger glyphicon glyphicon-trash'>Delete</a></p>
<hr>";
}
}
...
if(isset($_GET['nocv_id'])){
    $fullid=$_GET['nocv_id'];
    $full=mysqli_query($db,"SELECT * FROM contactuser WHERE nocv_id='$fullid'");
    if(mysqli_num_rows($full)>100){
        echo"<script>alert('Inbox is full delete messages')</script>";
    }
}
