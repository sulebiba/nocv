...
<div id="messages col-lg-6 col-md-3 col-sm-12 col-xs-12"class='comment col-lg-12 col-md-3 col-sm-12 col-xs-12'>
<h3><b><font face='Algerian'>Click to reply/Delete Invites</font></b></h3>
<hr>
<br>
<br>
<br>
...
$bitch=$_SESSION['nocv_id'];
 if(isset($_GET['nocv_id'])){
$nocv_id=$_GET['nocv_id'];
 }
if($bitch==$nocv_id){
$select=mysqli_query($db,"SELECT * FROM invites WHERE nocv_id='$nocv_id' ORDER BY 1 DESC LIMIT 0,200 ") or die(mysqli_error($db));
while($row=mysqli_fetch_array($select)){
    ...
    $id=$row['id'];
    $date=$row['date'];
$select_user=mysqli_query($db,"SELECT * FROM users WHERE nocv_id='$start_chat'") or die(mysqli_error($db));
    while($row=mysqli_fetch_array($select_user)){
    $name=$row['Fullname'];
    $ragner=$row['nocv_id']; 
    $selectno1=mysqli_query($db,"SELECT * FROM chat WHERE start_chat='$start_chat'") or die(mysqli_error($db));
$selectno2=mysqli_query($db,"SELECT * FROM chat_post WHERE start_chat='$start_chat'") or die(mysqli_error($db));
$selectno3=mysqli_query($db,"SELECT * FROM reply_post WHERE start_chat='$start_chat'") or die(mysqli_error($db));
$no1=mysqli_num_rows($selectno1);
$no2=mysqli_num_rows($selectno2);
$no3=mysqli_num_rows($selectno3);   
    echo"  
<p>Click <a href='start_a_chat.php?nocv_id=$nocv_id&start_chat=$start_chat&chat=$chat' class='btn btn-primary glyphicon glyphicon-ok'> here</a> to accept the invitation from <a href='profile.php?nocv_id=$ragner' class='btn btn-primary glyphicon glyphicon-user'>$name</a></p>
<p>$date</p>";
echo "
<p align='right'>($no1)($no2)($no3)</p>";
echo "
<p align='right'><a href='delete_invite.php?id=$id' class='btn btn-danger glyphicon glyphicon-trash'>Delete</a></p>
<hr>";
}
 }
 }
...
<div id="comment col-lg-12 col-md-3 col-sm-12 col-xs-12">
...
if(isset($_GET['nocv_id'])){
    $fullid=$_GET['nocv_id'];
    $full=mysqli_query($db,"SELECT * FROM invites WHERE nocv_id='$fullid'");
    if(mysqli_num_rows($full)>499){
        echo"<script>alert('Inbox is full delete invites')</script>";
    }
}
