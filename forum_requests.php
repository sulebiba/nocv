...
<h3><b><font face='Algerian'>Click to Accept/Delete Requests</font></b></h3>
<hr>
<br>
<br>
<br>
...
if($bitch==$nocv_id){
$select=mysqli_query($db,"SELECT * FROM requests WHERE start_chat='$nocv_id' ORDER BY 1 DESC LIMIT 0,200 ") or die(mysqli_error($db));
...
$select_user=mysqli_query($db,"SELECT * FROM users WHERE nocv_id='$nocv_id'") or die(mysqli_error($db));
    ...
    $ragner=$row['nocv_id']; 
    echo"  
<p>Click <a href='forum_requests.php?nocvi_id=$nocv_id&start_chat=$start_chat&chat=$chat' class='btn btn-primary glyphicon glyphicon-ok'> here</a> to accept <a href='profile.php?nocv_id=$ragner' class='btn btn-primary glyphicon glyphicon-user'>$name</a> Forum Request</p>
<p>$date</p>";

echo "
<p align='right'><a href='delete_foruminvite.php?id=$id' class='btn btn-danger glyphicon glyphicon-trash'>Delete</a></p>
<hr>";
}
 }
 }
...
//ob_end_flush();
$delete_request=mysqli_query($db,"delete from requests where nocv_id='$invite'") or die(mysqli_error($db));
header("Location:forum_requests.php?nocv_id=$way");
ob_end_flush();
}
}
?>
...
if(isset($_GET['nocv_id'])){
    $fullid=$_GET['nocv_id'];
    $full=mysqli_query($db,"SELECT * FROM requests WHERE start_chat='$fullid'");
    if(mysqli_num_rows($full)>499){
        echo"<script>alert('Inbox is full delete or accept Requests')</script>";
    }
}
